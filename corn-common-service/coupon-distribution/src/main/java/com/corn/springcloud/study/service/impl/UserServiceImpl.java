...
import com.corn.springcloud.study.constant.Constant;
import com.corn.springcloud.study.constant.CouponStatus;
import com.corn.springcloud.study.dao.CouponDao;
...
import com.corn.springcloud.study.feign.TemplateClient;
import com.corn.springcloud.study.service.IRedisService;
import com.corn.springcloud.study.service.IUserService;
import com.corn.springcloud.study.vo.*;
...
public class UserServiceImpl implements IUserService {
    ...
    public List<Coupon> findCouponsByStatus(Long userId, Integer status)
        ...
        //先从缓存里获取优惠券记录(如果为空，会设置空值用于处理缓存穿透)
        ...
        if (CollectionUtils.isNotEmpty(curCached)) {
            ...
            //如果为空，有可能用户第一次访问系统，或者缓存已经过期，需要从数据库获取
            ...
            // 填充 dbCoupons的 templateSDK 字段，然后通过feign远程访问
        ...
        preTarget = preTarget.stream()
                .filter(c -> c.getId() != -1)
                .collect(Collectors.toList());
        // 如果当前获取的是可用优惠券, 还需要做对已过期优惠券的延迟处理(已使用、已过期不用处理，直接返回)
    ...
    /**
     * <h2>根据用户 id 查找当前可以领取的优惠券模板(获取可以领取的模板列表)</h2>
     * @param userId 用户 id
    ...
    public Coupon acquireTemplate(AcquireTemplateRequest request)
        ...
        // 用户是否可以领取这张优惠券（获取用户已经拿到过的优惠券列表）
        List<Coupon> userUsableCoupons = findCouponsByStatus(
                request.getUserId(), CouponStatus.USABLE.getCode()
        );
        //根据模板id进行分组，看对应的模板，用户总共拿过多少张
        Map<Integer, List<Coupon>> templateId2Coupons = userUsableCoupons
                .stream()
                .collect(Collectors.groupingBy(Coupon::getTemplateId));
        //如果用户已经拿过这个优惠券，以及拿到的数量已经超过最大限制值，则不能再拿
